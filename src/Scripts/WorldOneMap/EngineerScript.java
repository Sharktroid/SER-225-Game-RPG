...
public class EngineerScript extends Script<NPC> {
    ...
    protected void setup() {
        if (isFlagSet("w1CuredNPC1") && isFlagSet("w1CuredNPC2") && isFlagSet("w1CuredNPC3") && isFlagSet("w1CuredNPC4") && isFlagSet("w1CuredNPC5")){
        ...
        if (isFlagSet("hasFinishedOMJ")) {
            ...
            } else if (isFlagSet("hasTalkedToNSE") && !isFlagSet("w1CuredAllNPCs")) {
                setNPCName(npcName);
            ...
            addTextToTextboxQueue("hasFinishedOMJ not set");
        }
    }
    ...
    public ScriptState execute() {
        if (isFlagSet("hasFinishedOMJ")) {
            ...
            return ScriptState.COMPLETED;
        }else if (!isFlagSet("hasFinishedOMJ")){
            start();
            if (isTextboxQueueEmpty()) {
                    end();
                    return ScriptState.COMPLETED;
